# Browser tasks: use jev-browser-relay

When a task needs a real browser and the `jev_browser_*` MCP tools are available, use them.
For reading a single static URL, fetch it directly instead.
...
Reading one static page is faster with an ordinary fetch. This runtime is for tasks that need a
real browser: interaction, logged-in state, forms, multi-step navigation.
